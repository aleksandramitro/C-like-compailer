...
void gen_sym_name_niz(int index1,int pozicija) {
      ...
      code("-%d(%%14)", (get_atr1(index1) * 4)*pozicija);
...
void gen_mov_niz(int input_index, int output_index, int pozicija) {
  code("\n\t\tMOV \t");
  if(get_kind(input_index) == NIZ) {
	gen_sym_name_niz(input_index,pozicija);
  } else {
  	gen_sym_name(input_index);
  }
  code(",");
  if(get_kind(output_index) == NIZ) {
	gen_sym_name_niz(output_index,pozicija);
  } else {
  	gen_sym_name(output_index);
  }
